...
LRESULT CALLBACK GridProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) 
	{
		...
		case BGM_SETFONT:
			BGHS[SelfIndex].hcolumnheadingfont = BGHS[SelfIndex].hfont = BGHS[SelfIndex].htitlefont = (HFONT)wParam;
    ...
    // SetWindowText(GetParent(hWnd),tbuffer);
		...
		case WM_CREATE:
              ...
              if(CountGrids()==1)
                  {
                   hfontbody=CreateFont(16,0,0, 0,100,FALSE,FALSE,FALSE,ANSI_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,VARIABLE_PITCH|FF_MODERN ,NULL);
                   hfontheader=CreateFont(18,0,0, 0,FW_HEAVY,FALSE,FALSE,FALSE,ANSI_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,VARIABLE_PITCH|FF_MODERN ,NULL);
                   hfonttitle=CreateFont(20,0,0, 0,FW_HEAVY,FALSE,FALSE,FALSE,ANSI_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,VARIABLE_PITCH|FF_MODERN ,NULL);
                  }
              ...
              if((BG_GridIndex >= 0)&&(BG_GridIndex < MAX_GRIDS))//if you aren't over the MAX_GRIDS limit, add a grid
                  {
                      ...
                      SendMessage(hWnd,WM_SETTEXT,0,(long)lpcs->lpszName);
...
int BinarySearchListBox(HWND lbhWnd,char* searchtext)
    {
     ...
     if(lbcount < 12)
         {
          ...
          FindResult = SendMessage(lbhWnd,LB_FINDSTRING,-1,(long) searchtext);
     ...
     //is it the head?
     SendMessage(lbhWnd,LB_GETTEXT,head,(long)headtext);
     headtext[9] = 0x00;
     ...
     //is it the tail?
     SendMessage(lbhWnd,LB_GETTEXT,tail,(long)tailtext);
     tailtext[9] = 0x00;
     ...
     while((!FOUND)&&((tail-head)>1))
         {
                 ...
                 SendMessage(lbhWnd,LB_GETTEXT,finger,(long)tbuffer);
                 tbuffer[9] = 0x00;

...
CMiniMapPointer::CMiniMapPointer(CScene *scene)
{
	...
	prog.insert_or_assign(MiniMap, new C3DObjectProgram("MiniMapPointer.v.glsl", "MiniMapPointer.f.glsl", "vertex", "texcoor", nullptr, nullptr));
...
void CMiniMapPointer::CreateBuffer(C3DObjectVBO *vbo_) {
	...
	if (vbo_ && Scene && Scene->MeshReady()) {
		...
		vbo_->vertices.get()->SetValues(0, glm::vec4(-meshSize.x, y, -meshSize.z, 1), glm::vec3(0, 1, 0), glm::vec4(1, 1, 1, 1), glm::vec2(1.0, 0.0));

		vbo_->vertices.get()->SetValues(1, glm::vec4(-meshSize.x, y, meshSize.z, 1), glm::vec3(0, 1, 0), glm::vec4(1, 1, 1, 1), glm::vec2(1.0, 1.0));

		vbo_->vertices.get()->SetValues(2, glm::vec4(meshSize.x, y, meshSize.z, 1), glm::vec3(0, 1, 0), glm::vec4(1, 1, 1, 1), glm::vec2(0.0, 1.0));

		vbo_->vertices.get()->SetValues(3, glm::vec4(meshSize.x, y, meshSize.z, 1), glm::vec3(0, 1, 0), glm::vec4(1, 1, 1, 1), glm::vec2(0.0, 1.0));

		vbo_->vertices.get()->SetValues(4, glm::vec4(meshSize.x, y, -meshSize.z, 1), glm::vec3(0, 1, 0), glm::vec4(1, 1, 1, 1), glm::vec2(0.0, 0.0));

		vbo_->vertices.get()->SetValues(5, glm::vec4(-meshSize.x, y, -meshSize.z, 1), glm::vec3(0, 1, 0), glm::vec4(1, 1, 1, 1), glm::vec2(1.0, 0.0));

		vbo_->vertices->usesCount = 1;
	}
}

glm::mat4 CMiniMapPointer::GetRotateMatrix(int vpId)
{
	return glm::rotate(Scene->Camera->GetAzimut() + (float)M_PI / 2, glm::vec3(0, 1, 0));
}

glm::mat4 CMiniMapPointer::GetTranslateMatrix(int vpId)
{
